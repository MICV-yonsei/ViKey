"""
ViKey inference on LongVideoBench using cached frame images.

This script is Stage 3 of the ViKey pipeline:
    Stage 1  add_VP.py / add_VP_outline.py        --  overlay visual prompts on frames
    Stage 2  keyword_extractor_longvideobench.py  --  extract question keywords
...
Example
-------
    python run_longvideobench.py \\
        --cache-dir /path/to/VP_frames \\
        --labels-path /path/to/longvideobench_labels.jsonl \\
...
- Run ``python run_longvideobench.py --help`` for the full flag list.
"""
...
def build_prompt(question: str, options: List[str]) -> str:
    ...
    parts.append(question_with_options)
...
def resolve_longvideobench_question_and_options(sample: Dict[str, Any]) -> (str, List[str]):
...
def get_processed_ids(pred_jsonl_path: str) -> set:
    processed_ids = set()
...
def resolve_video_id(sample: Dict[str, Any]) -> Optional[str]:
    for k in ["video_path", "videoPath", "video_filepath", "videoFilePath", "file_path", "filepath", "path"]:
...
def main():
    ...
    # clip_model, clip_preprocess = openai_clip.load("ViT-B/32", device=args.device)
    ...
    if args.labels_path:
        ...
        if args.save_jsonl:
            ...
            if args.overwrite:
                ...
                processed_ids = get_processed_ids(pred_jsonl_path)
                print(f"Resume mode: Found {len(processed_ids)} already processed items")
        ...
        for sample in tqdm(samples, desc="Processing JSONL", ncols=100):
            ...
            if not item_id:
                skipped += 1
                print(f"[WARN] No 'id' field found in sample")
                continue
            
            if item_id in processed_ids:
                resumed += 1
                if resumed <= 5:
                    print(f"[RESUME] Skipping already processed id: {item_id}")
                continue
            ...
            frames_cache_dir_by_id = os.path.join(cache_root, str(item_id))
            if os.path.exists(frames_cache_dir_by_id):
                frames_cache_dir = frames_cache_dir_by_id
                video_id = str(item_id)
                print(f"[INFO] Found cache directory by id: {frames_cache_dir}")
            else:
                if not video_path:
                    skipped += 1
                    print(f"[WARN] No 'video_path' field found in sample {item_id}, and id-based directory not found")
                    continue
                ...
                if not os.path.exists(frames_cache_dir):
                    ...
                    print(f"[WARN] Cache directory not found (tried id={item_id} and video_id={video_id}): {frames_cache_dir}")
                    continue
                else:
                    print(f"[INFO] Found cache directory by video_path: {frames_cache_dir}")

            question, options = resolve_longvideobench_question_and_options(sample)
            ...
            try:
                ...
                if args.save_visualization and save_dir and "frames_np" in out:
                    ...
                    frames_image_path = os.path.join(viz_frames_dir, f"{safe_id}_frames.png")
                    save_frames_visualization(out["frames_np"], frames_image_path, item_id, 
                ...
                print(f"[WARN] No frames for {item_id} (video: {video_id}): {e}")
            except Exception as e:
                skipped += 1
                missing_frames_list.append({
                    "id": item_id,
                ...
                print(f"[WARN] Failed on {item_id} (video: {video_id}): {e}")
