"""
ViKey Stage 1: render outline-style visual prompts as text on cached frames.
...
The caption is rendered as **red text with a white stroke** (no background
fill) -- the "outline" variant of ViKey's visual prompt. Use ``add_VP.py``
instead for the background-box variant.
...
def add_text_to_image(
    ...
    The text is drawn with a stroked outline so it stays legible regardless of
    the underlying frame content. Font size is auto-derived from the image's
    shorter side (``min(width, height) // auto_font_div``) when ``font_size``
    is ``None``; stroke width defaults to ``max(1, font_size // 10)``.
...
def _build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Add numeric "frame #NN" captions (outline / stroked-text style) to images.'
    )
    ...
    parser.add_argument('--stroke_color', type=str, default=None,
                        help='Stroke color "r,g,b" (default: "255,255,255" -- white).')
    parser.add_argument('--stroke_width', type=int, default=None,
                        help='Stroke width in pixels (default: font_size // 10).')
    return parser
...
if __name__ == "__main__":
    ...
    if args.stroke_color:
        parsed = parse_color(args.stroke_color)
        if len(parsed) != 3:
            raise ValueError("--stroke_color must be in R,G,B format (3 values)")
        stroke_color = parsed  # type: ignore[assignment]
