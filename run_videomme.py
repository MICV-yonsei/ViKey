"""
ViKey inference on Video-MME using cached frame images.

This script is Stage 3 of the ViKey pipeline:
    Stage 1  add_VP.py / add_VP_outline.py  --  overlay visual prompts on frames
    Stage 2  keyword_extractor_videomme.py  --  extract question keywords
...
-------
    python run_videomme.py \\
        ...
        --labels-path /path/to/videomme_labels.jsonl \\
...
- Run ``python run_videomme.py --help`` for the full flag list.
"""
...
def build_prompt(question: str, options: List[str]) -> str:
    """
    Build Video MME format prompt (identical to lmms-eval).
    
    Structure: pre_prompt + question + options + post_prompt
    - pre_prompt: "" (empty in Video MME)
    - question: User question
    - options: Choice list (A, B, C, D)
    - post_prompt: "The best answer is:"
    
    Args:
        question: User question text
        options: Choice list, e.g., ["A. Disgust.","B. Joy.","C. Sadness.","D. Calmness."]
    ...
    prompt_parts = []
    if pre_prompt:
        prompt_parts.append(pre_prompt)
    prompt_parts.append(question_with_options)
    if post_prompt:
...
def load_video_uniform(video_path: str, max_frames: int, force_sample: bool = False):
    ...
    frames_np = vr.get_batch(idx).asnumpy().astype(np.uint8)  # (F, H, W, C)
    return frames_np, frame_time_str, video_time
...
def resolve_video_id(sample: Dict[str, Any]) -> Optional[str]:
    """
    Resolve video ID from sample with priority order.
    """
    for k in ["video_id"]:
...
def main():
    ...
    if args.labels_path:
        ...
        for sample in tqdm(samples, desc="Processing JSONL", ncols=100):
            ...
            if not question_id and not video_id:
                skipped += 1
                print(f"[WARN] No question_id or video_id found in sample")
                continue
            
            resume_id = question_id if question_id else video_id
            if resume_id and resume_id in processed_question_ids:
                resumed += 1
                continue
            ...
            if question_id:
                candidate_dir = os.path.join(cache_root, question_id)
                if os.path.exists(candidate_dir):
                    frames_cache_dir = candidate_dir
                    used_id = question_id
                    print(f"[DEBUG] Found directory using question_id: {candidate_dir}")
                else:
                    print(f"[DEBUG] Tried question_id path but not found: {candidate_dir} (cache_root={cache_root}, question_id={question_id})")
            
            if not frames_cache_dir and video_id:
                candidate_dir = os.path.join(cache_root, video_id)
                if os.path.exists(candidate_dir):
                    frames_cache_dir = candidate_dir
                    used_id = video_id
                    print(f"[DEBUG] Found directory using video_id: {candidate_dir}")
            ...
            if not frames_cache_dir:
                ...
                missing_frames_list.append({
                    ...
                    "tried_paths": {
                        "question_id_path": os.path.join(cache_root, question_id) if question_id else None,
                        "video_id_path": os.path.join(cache_root, video_id) if video_id else None,
                        "cache_root": cache_root
                    }
                })
                print(f"[WARN] Cache directory not found for question_id={question_id}, video_id={video_id}")
                print(f"[WARN] Tried paths: question_id={os.path.join(cache_root, question_id) if question_id else None}, video_id={os.path.join(cache_root, video_id) if video_id else None}")
                continue

            question, options = resolve_question_and_options(sample)
            ...
            try:
                ...
                if args.save_visualization and save_dir and "frames_np" in out:
                    ...
                    display_id = used_id if used_id else (question_id if question_id else video_id)
                    frames_image_path = os.path.join(viz_frames_dir, f"{display_id}_frames.png")
                ...
                display_id = used_id if used_id else (question_id if question_id else video_id)
                print(f"[WARN] No frames in {display_id}: {e}")
            except Exception as e:
                skipped += 1
                missing_frames_list.append({
                    "question_id": question_id,
                ...
                display_id = used_id if used_id else (question_id if question_id else video_id)
                print(f"[WARN] Failed on {display_id}: {e}")
        ...
        if missing_frames_list:
            ...
            with open(missing_json_path, "w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "missing_question_ids": missing_frames_list
            ...
            with open(missing_txt_path, "w", encoding="utf-8") as f:
                ...
                for item in missing_frames_list:
                    f.write(f"Question ID: {item.get('question_id', 'N/A')}\n")
                    if 'video_id' in item:
                        f.write(f"Video ID: {item['video_id']}\n")
                    if 'used_id' in item:
                        f.write(f"Used ID: {item['used_id']}\n")
    ...
    # JSONL mode only - single question mode is not supported
